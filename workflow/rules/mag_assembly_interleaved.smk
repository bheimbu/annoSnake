localrules: MAG_checkm2_interleaved, MAG_checkm2_bins, MAG_above_threshold_bins
...
rule MAG_coverm:
      ...
      shell:
        ...
        if compgen -G "{OUTDIR}/MAGs/above_threshold_bins/{wildcards.sample}/*.fa" > /dev/null; then
            coverm genome --interleaved {INPUTDIR}/{wildcards.sample}.fastq.gz --genome-fasta-files {OUTDIR}/MAGs/above_threshold_bins/{wildcards.sample}/*fa --threads {threads} >& {OUTDIR}/MAGs/coverm/{wildcards.sample}/{wildcards.sample}.abundance
        else
            touch "{OUTDIR}/MAGs/coverm/{wildcards.sample}/.rule_completed"
        fi
        """

rule MAG_checkm2_interleaved:
      input:
      ...
      benchmark:
        OUTDIR/ "benchmarks/MAG_checkm2_interleaved.txt"
      shell:
        """
        mkdir -p {output}
