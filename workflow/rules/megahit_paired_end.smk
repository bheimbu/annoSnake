...
rule check_input:
    ...
    shell:
        ...
        if [ -e {INPUTDIR}/{wildcards.sample}_R1.fastq.gz ] && [ -e {INPUTDIR}/{wildcards.sample}_R2.fastq.gz ]; then
            ...
            echo "Please make sure both R1 and R2 reads are present in the input directory."
            exit 1
        fi
        """
...
rule megahit:
    ...
    conda:
        "envs/mags.yaml"
    benchmark:
        OUTDIR/ "benchmarks/{sample}_megahit.txt"
    shell:
        """
...
rule preprocess:
    ...
    benchmark:
        OUTDIR/ "benchmarks/{sample}_preprocess.txt"
    shell: 
	    """
