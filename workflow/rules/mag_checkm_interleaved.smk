...
rule MAG_checkm_interleaved1:
      ...
      shell:
        ...
        if [ -e {OUTDIR}/"MAGs/above_threshold_bins/{wildcards.sample}/{wildcards.sample}_bin.1.fa" ]; then
            ...
            coverm genome --interleaved {INPUTDIR}/{wildcards.sample}.fastq.gz --genome-fasta-files {OUTDIR}/MAGs/above_threshold_bins/{wildcards.sample}/*fa --threads {threads} >& {OUTDIR}/MAGs/checkm/{wildcards.sample}/{wildcards.sample}.abundance
        else
