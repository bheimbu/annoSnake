rule MAG_microbeannotator:
      input:
          gtf=expand(OUTDIR/ "taxonomy/prokka/{sample}/{sample}.gtf", sample=SAMPLES),
      ...
      shell:
          """
          microbeannotator -i $(ls MAGs/prokka/*/*.faa) -d {params.db} -o {params.dir} -m diamond -p 2 -t {threads} --light
